...
from app.hr.trusted_rag import policy_state
...
class OrganizationMemory:
    ...
    async def publish(
        ...
        item = {
            ...
            "access_scope": access_scope or ["employee", "hr", "hr_admin", "admin"],
    ...
    async def recall(
        self, query: str, dept_ids: list[str], role: str = "employee", limit: int = 8
    ) -> list[dict[str, Any]]:
    ...
    async def source_chunks(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            for ref in item.get("source_refs") or []:
                ...
                if chunk and doc and doc.get("status") == "active" and policy_state(doc) == "effective" and doc.get("version") == ref.get("document_version"):
    ...
    async def _verify_sources(self, refs: list[dict[str, Any]], dept_id: str = "") -> list[dict[str, Any]]:
        ...
        for ref in refs:
            ...
            if not doc or not chunk or doc.get("status") != "active" or policy_state(doc) != "effective":
                continue
