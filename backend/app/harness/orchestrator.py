...
class Orchestrator:
    ...
    async def answer(
        ...
        user_row = await self.store.get("users", user_id) or {"_id": user_id, "role": "employee"}
        if "id" not in user_row:
            user_row["id"] = user_row.get("_id", user_id)
        access_context = AccessContext.from_user(user_row, dept_ids=dept_ids or [])
        ...
        # Memory ACL role comes from authenticated identity, never from model-predicted intent.
        memory_context = await self.memory_context_builder.build(
            session_id, user_id, query, resolved_depts, role=access_context.role, include_organization=True
        )
        ...
        if self.dept_agent_client is not None and self.dept_agent_client.enabled and resolved_depts:
            ...
            if sub_answers:
                degraded: list[str] = []
                if failed:
                    fallback_chunks = await self.retrieval_agent.retrieve(
                        [query], failed, top_k=self.settings.hybrid_topk, access_context=access_context
                    )
        ...
        # 6. Retrieval
        # `resolved_depts` is a retrieval namespace, not an authorization grant.
        # TrustedPolicyRAG keeps the identity departments captured from the authenticated user.
        chunks = await self.retrieval_agent.retrieve(
            skill_plan.queries, resolved_depts, top_k=skill_plan.top_k, access_context=access_context
        )
        ...
        for retrieved in retrieved_chunks:
            ...
            if chunk_id and chunk_id not in seen_chunks:
                ...
                seen_chunks.add(chunk_id)
        if self.retrieval_agent.trusted_rag is not None:
            chunks = await self.retrieval_agent.trusted_rag.filter_chunks(chunks, access_context)
