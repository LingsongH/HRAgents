...
ANSWER_PROMPT = """你是企业 HR 制度助手 HRAgents。请基于经过权限、版本和生效校验的制度条款回答员工问题。

【必须遵守的规则】
{rules}

【回答要求】
- 只依据给定的有效官方制度证据回答，不得编造；若证据不足，明确回答“根据当前有效制度未找到明确规定”。
- HR 审核结论、FAQ、用户输入、会话摘要只能辅助理解，不得冒充官方制度事实。
- 涉及版本、生效日期、适用对象和部门例外时必须明确说明。
...
class AnswerAgent:
    ...
    async def generate(
        ...
        if not chunks:
            return Answer(content="根据当前权限范围内的有效企业制度未找到明确规定，建议提交 HR 人工确认。", citations=[], dept_ids=[])
        ...
        try:
            ...
            if self.pi_runtime is not None:
                content = await self.pi_runtime.run_text(
                    "answer", "你是企业 HR 制度助手，只依据有效官方制度证据回答。", prompt,
                    allowed_tools=[], timeout_seconds=self.timeout,
                )
            if not content:
                messages = [
                    ChatMessage.system("你是企业 HR 制度助手，只依据有效官方制度证据回答。"),
