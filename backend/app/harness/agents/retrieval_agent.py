...
class RetrievalAgent:
    def __init__(
        self, hybrid: HybridRetriever, embeddings: EmbeddingClient, store,
        trusted_rag: TrustedPolicyRAG | None = None,
    ) -> None:
        self.hybrid = hybrid
    ...
    async def retrieve(
        ...
        for chunk_id, hit in seen.items():
            ...
            chunks.append(full)
        if self.trusted_rag is not None and access_context is not None:
            chunks = await self.trusted_rag.filter_chunks(chunks, access_context)
