...
class Indexer:
    ...
    async def ingest(self, file_path: Union[str, Path], dept_id: str, uploaded_by: str = "system") -> dict[str, Any]:
        ...
        # 4. 元数据提取（LLM，可选）
        meta: dict[str, Any] = {
            "effective_date": None, "expiry_date": None, "doc_type": "other",
            "policy_level": "company", "keywords": [], "applicable_scope": ["all"],
            "allowed_roles": ["employee", "hr", "hr_admin", "admin"],
            "cross_refs": [], "parent_policy_title": "",
        }
        ...
        doc: dict[str, Any] = {
            ...
            "status": "indexing",
            "policy_state": "review",
            "policy_level": meta.get("policy_level", "company"),
            "parent_policy_id": None,
            "parent_policy_title": meta.get("parent_policy_title", ""),
            "visibility": "company",
            "allowed_dept_ids": [],
            "allowed_roles": meta.get("allowed_roles", ["employee", "hr", "hr_admin", "admin"]),
        ...
        try:
            ...
            await self.store.update_document(
                doc_id, {
                    "status": "active", "policy_state": "effective",
                    "vector_status": "ready", "chunk_count": len(stored_chunks)
                }
            )
        except Exception:
            # 避免失败入库留下可检索的半成品。
        ...
        for old in same_title:
            if old.get("status") == "active":
                await self.store.update_document(old["_id"], {
                    "status": "archived", "policy_state": "deprecated", "updated_at": now_iso()
                })
    ...
    async def set_status(self, doc_id: str, status: str) -> dict[str, Any]:
        ...
        if status == "active":
            await self.store.update_document(doc_id, {
                "status": status, "policy_state": "effective", "updated_at": now_iso()
            })
            return await self.reindex(doc_id)
        await self.store.update_document(doc_id, {
            "status": status,
            "policy_state": "deprecated" if status == "archived" else status,
            "updated_at": now_iso(),
        })
