...
# 内置跨职能 Hook：HR 制度问题可按关键词补充财务、法务、研发或行政检索范围
DEFAULT_HOOKS = [
    {
        "_id": "hook_policy_cross_function",
        "name": "policy_cross_function_hook",
        "dept_id": "",
        "scope": "global",
        "trigger": {
            "intent_patterns": ["制度冲突", "报销", "劳动合同", "弹性工作"],
            "keyword_any": ["冲突", "财务", "法务", "研发", "HR"],
            "min_depts": 1,
        },
...
class HookEngine:
    ...
    async def apply(self, hooks: list[dict[str, Any]], intent: Intent, dept_ids: list[str]) -> list[str]:
        ...
        kw_dept = {
            "人事": "dept_hr", "HR": "dept_hr", "招聘": "dept_hr",
            "报销": "dept_finance", "预算": "dept_finance", "财务": "dept_finance",
            "法务": "dept_legal", "合规": "dept_legal", "冲突": "dept_legal",
            "研发": "dept_rd", "弹性工作": "dept_rd",
