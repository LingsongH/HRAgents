...
仅输出 JSON：
{{
  "root_causes": {{"retrieval": 0, "intent": 0, "generation": 0, "knowledge_gap": 0, "policy_hierarchy": 0, "acl_version": 0}},
  "suggestions": [
...
- knowledge_gap：有效官方制度里确实没有这个内容
- policy_hierarchy：把上位制度授权下的部门例外/补充错误判断为冲突，或忽略 parent_policy / policy_level
- acl_version：使用了无权限、已废止、尚未生效或版本错误的制度证据
"""
...
class LoopEngine:
    ...
    def _next_action(observed: int, adaptations: list[dict[str, Any]]) -> str:
        if observed == 0:
            return "先在员工端完成几次制度问答或 HR 审核并提交反馈，再触发 Loop 查看策略变化。"
        if adaptations:
    ...
    def _heuristic_root_causes(bad_cases: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for c in bad_cases:
            ...
            explicit = str(detail.get("loop_category") or detail.get("reason") or "").lower()
            if "hierarchy" in explicit or "parent_policy" in explicit:
                causes["policy_hierarchy"] += 1
            elif any(token in explicit for token in ("acl", "version", "deprecated", "effective")):
                causes["acl_version"] += 1
            elif c.get("signal") == "verifier_fail":
                causes["generation"] += 1
