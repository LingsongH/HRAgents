...
class SkillExecutor:
    ...
    async def matching(self, query: str, dept_ids: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        exclusive = [s for s in matched if s.get("exclusive")]
        selected = exclusive or matched
        return sorted(selected, key=lambda s: int(s.get("priority", 0)), reverse=True)
    ...
    def _execute_workflow(self, plan: SkillPlan, skill: dict[str, Any], query: str) -> None:
        ...
        for step in action.get("steps") or []:
            ...
            if kind == "retrieve":
                ...
                plan.instructions.append("回答工作日、法定假期或 HR 截止时间时必须结合企业工作日历，并明确给出日期依据。")
