...
from app.hr.interview_agent import InterviewAgent
from app.hr.policy_claims import PolicyClaimExtractor
from app.hr.trusted_rag import TrustedPolicyRAG
...
class Container:
    ...
    trusted_policy_rag: TrustedPolicyRAG
    policy_claim_extractor: PolicyClaimExtractor
    policy_conflict_workflow: PolicyConflictWorkflow
    bounded_react_runtime: BoundedReActRuntime
...
def build_container(settings: Optional[Settings] = None) -> Container:
    ...
    trusted_policy_rag = TrustedPolicyRAG(store)
    policy_claim_extractor = PolicyClaimExtractor(store, llm, embeddings)
    policy_conflict_workflow = PolicyConflictWorkflow(
        store, llm, embeddings, policy_claim_extractor, trusted_policy_rag
    )
    bounded_react_runtime = BoundedReActRuntime(max_steps=6, tool_budget=4, timeout_seconds=20.0)
    interview_agent = InterviewAgent(store, llm, bounded_react_runtime)
    ...
    retrieval_agent = RetrievalAgent(hybrid, embeddings, store, trusted_policy_rag)
    ...
    return Container(
        ...
        trusted_policy_rag=trusted_policy_rag,
        policy_claim_extractor=policy_claim_extractor,
        policy_conflict_workflow=policy_conflict_workflow,
        bounded_react_runtime=bounded_react_runtime,
