...
async def test_intent_prefers_pi_without_calling_local_llm():
    store = MemoryStore()
    await store.upsert_department({"_id": "dept_hr", "name": "人力资源部"})
    local = FakeLLM(json_result={"type": "other"})
    pi = FakePi(json_result={
        "type": "employee_qa", "depts": ["dept_hr"], "user_role": "employee",
        "entities": {"matter": "请假"}, "needs_cross_dept": False, "confidence": 0.9,
    })
    intent = await IntentAgent(local, store, pi, 1.0).infer("请假制度", "u1")
    assert intent.type == "employee_qa"
...
async def test_rewriter_falls_back_to_local_when_pi_unavailable():
    ...
    queries = await QueryRewriter(local, store, pi, 1.0).rewrite("请假")
    assert queries == ["本地改写"]
...
async def test_answer_and_verifier_use_pi_outputs():
    store = MemoryStore()
    chunk = {
        "_id": "d1:0", "doc_id": "d1", "dept_id": "dept_hr",
        "chunk_index": 0, "content": "请假截止第八周。", "section_path": [],
    }
    local = FakeLLM(text_result="local answer", json_result={"passed": False})
    pi_answer = FakePi(text_result="请假截止第八周。[来源1]")
    answer = await AnswerAgent(local, store, pi_answer, 5.0).generate("请假如何办理", [chunk])
    assert answer.content.startswith("请假截止第八周")
    assert local.calls == 0

    pi_verify = FakePi(json_result={"passed": True, "score": 0.95, "issues": []})
    verdict = await VerifierAgent(local, pi_verify, 3.0).verify("请假如何办理", answer, [chunk])
