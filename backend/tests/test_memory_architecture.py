"""企业 Fact Plane + 分层 Memory 的核心不变量。"""
...
async def _seed_fact(container, doc_id="doc1", dept_id="dept_hr", version="1.0"):
    await container.store.insert_document({
        "_id": doc_id, "company_id": "default", "dept_id": dept_id,
        "title": "员工考勤管理办法", "version": version, "status": "active", "policy_state": "effective",
    })
    await container.store.insert_chunks([{
        "_id": f"{doc_id}:0", "doc_id": doc_id, "dept_id": dept_id,
        "chunk_index": 0, "content": "员工请假应按公司流程提交审批。",
        "section_path": ["请假"], "section_title": "请假审批",
    }])
...
async def test_fact_plane_only_returns_active_scoped_chunks(fresh_container):
    c = fresh_container
    await _seed_fact(c)
    assert await c.fact_plane.active_chunk("doc1:0", ["dept_hr"])
    assert await c.fact_plane.active_chunk("doc1:0", ["dept_finance"]) is None
    await c.store.update_document("doc1", {"status": "archived", "policy_state": "deprecated"})
...
async def test_organization_memory_requires_and_rechecks_sources(fresh_container):
    ...
    with pytest.raises(ValueError, match="官方来源"):
        await c.organization_memory.publish("department", "faq", "请假", "走审批", [], dept_id="dept_hr")
    item = await c.organization_memory.publish(
        "department", "faq", "请假审批", "员工请假应按公司流程提交审批。",
        [{"doc_id": "doc1", "chunk_id": "doc1:0", "document_version": "1.0"}], dept_id="dept_hr",
    )
    assert (await c.organization_memory.recall("请假审批", ["dept_hr"]))[0]["_id"] == item["_id"]
    assert await c.organization_memory.recall("请假审批", ["dept_finance"]) == []
    await c.store.update_document("doc1", {"status": "archived", "policy_state": "deprecated"})
    assert await c.organization_memory.recall("请假审批", ["dept_hr"]) == []
...
async def test_user_memory_consent_sensitivity_revision_and_forget(fresh_container):
    ...
    candidate = await c.user_semantic_memory.remember("u1", "answer_style", "concise", source_type="inferred", consent=False)
...
async def test_working_memory_stores_chunk_ids_and_context_respects_budget(fresh_container):
    c = fresh_container
    await c.working_memory.append_message("s1", "user", "请假如何办理")
    await c.working_memory.set_intent("s1", {"depts": ["dept_hr"], "entities": {"matter": "请假"}})
    ...
    context = await c.memory_context_builder.build("s1", "u1", "请假", ["dept_hr"])
    assert context.session["entities"]["matter"] == "请假"
...
async def test_episodic_sequence_history_and_retention(fresh_container):
    ...
    await c.store.upsert("conversation_events", {
...
async def test_hot_topics_use_atomic_aggregate(fresh_container):
    c = fresh_container
    await c.dept_memory.bump_hot_query("dept_hr", "请假审批")
    await c.dept_memory.bump_hot_query("dept_hr", "请假审批")
    topics = await c.store.find("memory_topics", {"dept_id": "dept_hr"})
...
async def test_hot_topics_concurrent_increment(fresh_container):
    c = fresh_container
    await asyncio.gather(*(c.dept_memory.bump_hot_query("dept_hr", "请假审批") for _ in range(20)))
    topics = await c.store.find("memory_topics", {"dept_id": "dept_hr"})
