...
def test_user_memory_api_ownership_and_sensitive_rejection():
    with TestClient(app) as client:
        token = _login(client, "employee", "employee123")
...
def test_employee_cannot_publish_organization_memory():
    with TestClient(app) as client:
        token = _login(client, "employee", "employee123")
        response = client.post(
            "/api/v1/memory/organization",
            headers={"Authorization": f"Bearer {token}"},
            json={"scope": "department", "dept_id": "dept_hr", "title": "FAQ", "content": "内容"},
        )
...
def test_admin_system_insights_and_department_scope():
    with TestClient(app) as client:
        ...
        dept_admin = _login(client, "hr_admin", "admin123")
