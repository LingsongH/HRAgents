...
async def test_mongo_increment_omits_conflicting_parent_default() -> None:
    ...
    await store.increment(
        "user_profiles", "employee", "prefs.intent_counts.hr_process", 1,
        {"user_id": "employee", "role": "employee", "prefs": {}, "feedback_counts": {}},
    )

    assert mongo.target.update == {
        "$inc": {"prefs.intent_counts.hr_process": 1},
        "$setOnInsert": {
            "user_id": "employee", "role": "employee", "feedback_counts": {},
        },
    }
...
async def test_mongo_increment_keeps_non_overlapping_defaults() -> None:
    ...
    await store.increment("user_profiles", "employee", "query_count", 1, {"prefs": {}})
