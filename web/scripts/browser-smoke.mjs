...
const roles = [
  { key: "employee", username: "employee", password: "employee123", expected: "制度问答工作台" },
...
async function run(role, port) {
  ...
  try {
    ...
    if(role.key!=="employee"){
      ...
      await writeFile("/tmp/hragents-employee-welcome.png",Buffer.from(welcomeShot.data,"base64"));
